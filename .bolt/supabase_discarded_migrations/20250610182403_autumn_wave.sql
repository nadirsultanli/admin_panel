/*
  ...
  1. Security Functions
     - Add helper functions for security checks (is_admin, auth_user_owns_record)
...
BEGIN
  ...
  -- Create user cancel policy if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can cancel own orders' AND tablename = 'orders' AND schemaname = 'public') THEN
